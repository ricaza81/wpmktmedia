...
this.$importBtnCopy.prop('disabled',!this.getValue());clearTimeout(pid)}.bind(this),1);this.$container.find('.us-builder-states').toggleClass('hidden',!this.extStates.length);this.$container.on('click','[data-accordion-id]',this._events.toggleAccordion.bind(this)).on('click','.usof-design-options-reset',this._events.resetValues.bind(this)).on('click','.usof-design-options-responsive',this._events.toggleResponsive.bind(this)).on('click','[data-responsive-state]',this._events.changeResponsiveStates.bind(this));this.$document.on('usb.setResponsiveState',this._events.usbSetResponsiveState.bind(this));this.$import.on('click','button[data-action]',this._events.importActions.bind(this))},_events:{changeValue:function(field){var resultValue={},valueStateChanged={};$.each(this.groupParams,function(responsiveState,groupParams){var groupValues=groupParams.getValues();$.each(groupValues,function(param,value){var defaultValue=this.defaultValues[responsiveState][param];if(['position','border-style','text-align'].indexOf(param)>-1&&responsiveState!=='default'){if(valueStateChanged[param]=(value!==defaultValue||valueStateChanged[param])){defaultValue=null}
if(param==='text-align'&&!value&&!!valueStateChanged[param]){value='inherit'}}
...
var pid=setTimeout(function(){$groupHeader.removeClass('changed');clearTimeout(pid)},1000*0.5)},watchAttrLink:function(field,e,state){var $target=$(e.currentTarget),isUnlink=$target.hasClass('fa-unlink'),relations=[];if(state!==undefined){isUnlink=state}

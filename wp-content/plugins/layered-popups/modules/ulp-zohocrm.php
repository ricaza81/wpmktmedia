			...
			<div id="ulp-zohocrm-connection">
				<table class="ulp_useroptions">
					<tr>
						...
						<td>
							<a class="ulp-button ulp-button-small" onclick="window.open(\''.admin_url('admin-ajax.php').'?action=ulp-zohocrm-connect\', \'_blank\', \'height=560,width=720,menubar=no,scrollbars=no,status=no,toolbar=no\'); return false;"><i class="fa fa-check"></i><label>'.__('Connect to Zoho CRM', 'ulp').'</label></a>
			...
			<div id="ulp-zohocrm-connection">
				<table class="ulp_useroptions">
					<tr>
						...
						<td>
							<a class="ulp-button ulp-button-small" onclick="return ulp_zohocrm_disconnect(this);"><i class="fa fa-close"></i><label>'.__('Disconnect from Zoho CRM', 'ulp').'</label></a>
		...
		if (current_user_can('manage_options')) {
			...
			if (array_key_exists("do", $_REQUEST)) {
				switch ($_REQUEST['do']) {
					...
					case 'connect':
						if (array_key_exists("code", $_REQUEST)) {
							...
							if (is_array($result) && array_key_exists('refresh_token', $result)) {
								...
								$content = __('Invalid connection credentials. Please try again.', 'ulp').'<div><a class="button" href="'.admin_url('admin-ajax.php').'?action=ulp-zohocrm-connect"><i class="fa fa-check"></i><label>'.__('Try again', 'ulp').'</label></a></div>';
							} else {
								$content = __('Something went wrong. We got unexpected server response. Please try again.', 'ulp').'<div><a class="button" href="'.admin_url('admin-ajax.php').'?action=ulp-zohocrm-connect"><i class="fa fa-check"></i><label>'.__('Try again', 'ulp').'</label></a></div>';
							}
						} else {
							$content = __('Something went wrong. We got unexpected server response. Please try again.', 'ulp').'<div><a class="button" href="'.admin_url('admin-ajax.php').'?action=ulp-zohocrm-connect"><i class="fa fa-check"></i><label>'.__('Try again', 'ulp').'</label></a></div>';
						}
					...
					default:
						$content = __('Inavlid URL. Please try again.', 'ulp').'<div><a class="button" href="'.admin_url('admin-ajax.php').'?action=ulp-zohocrm-connect"><i class="fa fa-check"></i><label>'.__('Try again', 'ulp').'</label></a></div>';
						break;
				}
				echo '
<!DOCTYPE html>
...
<body>
	<div class="main-container">
		...
		<div class="button-container">
			<a class="button" href="#" onclick="return connect(this);"><i class="fa fa-check"></i><label>Connect to Zoho CRM</label></a>			
		</div>
	</div>
</body>
</html>';
